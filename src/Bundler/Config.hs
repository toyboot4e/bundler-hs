  ...
  vsep
    ( map
        ...
        [ "--rename-cmd is queried in lockstep via stdin/stdout, one line per name:",
          "",
          "    kind <TAB> module <TAB> name <TAB> default-suffix  ->  new-name",
          "",
          "  kind is value, type, con, field, op, or extmod.",
          "  `echo \"$name$suffix\"` reproduces the default behavior. The suffix comes",
          "  last because it is empty for a name the default rule leaves as it is.",
          "",
          "  Example script:",
          "",
          "    #!/bin/sh",
